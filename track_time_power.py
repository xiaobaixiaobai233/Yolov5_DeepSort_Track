...
# ---------------------- 功耗采集模块 ----------------------
class PowerMonitor:
    def __init__(self, csv_path="power_data.csv", sample_interval=0.1):  # 修改为2秒
        self.csv_path = csv_path
        self.sample_interval = sample_interval
        self.device = Device(0)  # 0号GPU（多卡可调整）
        self.running = False
        self.thread = None
        # 初始化CSV文件
        with open(self.csv_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["timestamp", "gpu_power_w", "frame_idx"])  # 时间戳、GPU功耗、帧索引
    ...
    def _monitor(self):
        while self.running:
            try:
                ...
                gpu_power = self.device.power_usage()  # GPU实时功耗（W）
                # 写入CSV（追加模式）
                with open(self.csv_path, "a", newline="", encoding="utf-8") as f:
                    writer = csv.writer(f)
                    writer.writerow([timestamp, gpu_power, getattr(self, "current_frame", -1)])
...
def detect(opt, save_img=False):
    ...
    # 停止功耗采集
    power_monitor.stop()
    print(f"功耗数据已保存至: {power_monitor.csv_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fourcc', type=str, default='FLV1',
