...
需要用到yolov5原预训练模型yolov5s.pt和行人重识别模型ckpt.t7，代码上传限制文件大小。

### （2）根据流程里的步骤下载虚拟环境和各类配置

### （3）接下来，我们需要修改track.py文件中的参数：
我们需要将参数修改为我们自己的视频文件路径和yolov5的预训练模型路径。我们还需要将编码方式设置为mp4v，这样保存的mp4文件才能打开。我们可以使用以下命令运行。
